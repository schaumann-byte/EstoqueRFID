...
export default function OrderDetailClient({
  ...
  const [data, setData] = useState<OrderDetail>(initialData);
  ...
  // Usuário logado (você deve pegar do contexto de autenticação)
  const vinculadoPor = "Sub Ten Gutierres"; // TODO: pegar do contexto de auth

  const refreshData = () => {
    router.refresh();
  };

  const handleOpenModal = (line: {
    pedido_item_id: number;
    codigo_produto: number;
    produto_descricao: string;
  }) => {
    setSelectedLine({
      pedidoItemId: line.pedido_item_id,
      codigoProduto: line.codigo_produto,
      produtoDescricao: line.produto_descricao,
    });
    setModalOpen(true);
  };

  const handleVinculoSuccess = () => {
    refreshData();
    setMessage({ type: "success", text: "Item vinculado com sucesso!" });
    setTimeout(() => setMessage(null), 5000);
  };

  const handleDesvincular = async (pedidoItemId: number, itemId: number, rfid: string) => {
    if (!confirm(`Deseja realmente desvincular o item ${rfid}?`)) {
      return;
    }
    ...
    try {
      ...
      const url = `${base.replace(/\/$/, "")}/vinculos/${pedidoItemId}/item/${itemId}`;

      const res = await fetch(url, { method: "DELETE" });

      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.detail || "Erro ao desvincular item");
      }

      setMessage({ type: "success", text: "Item desvinculado com sucesso!" });
      setTimeout(() => setMessage(null), 5000);
      
      refreshData();
    } catch (err: any) {
      setMessage({ type: "error", text: err.message || "Erro ao desvincular item" });
      setTimeout(() => setMessage(null), 5000);
    }
  };
    ...
    <main className="flex-1 p-6 md:p-8 overflow-y-auto">
                    ...
                    >
                            ...
                            {ln.vinculos.map((v) => (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  className="ml-2 opacity-0 group-hover:opacity-100 text-rose-600 hover:text-rose-700 transition-opacity"
                                  title="Desvincular"
                                >
                                  <Trash2 size={12} />
                                </button>
                      ...
                      <td className="px-3 text-center align-middle">
                        <button
                          ...
                          onClick={() =>
                            handleOpenModal({
                              pedido_item_id: ln.pedido_item_id,
                              codigo_produto: ln.codigo_produto,
                              produto_descricao: ln.produto_descricao,
                            })
                          }
      ...
      {selectedLine && (
        <VincularModal
          ...
          pedidoId={header.id}
          codigoProduto={selectedLine.codigoProduto}
          produtoDescricao={selectedLine.produtoDescricao}
